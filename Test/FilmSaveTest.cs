...
public class FilmSaveTest
{
    ...
    [DataTestMethod]
    [DataRow("Test film", 1, 1, 1, "dr. Good", "mr. Good", "hello", "Good@gmail.com")]
    [DataRow("Test film2", 2, 3, 5, "dr. Evil", "mr. Evil", "hello", "Evil@gmail.com")]
    public void AddCustomerToFilmTest(string title, int duration, double price, double rating, string name, string username, string password, string email)
    {
        Customer to_add = new Customer(name, username, password, email);
        Film film = new Film(title, duration, price, rating);
